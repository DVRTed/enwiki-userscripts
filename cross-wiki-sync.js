...
(() => {
    ...
    const is_own_script = page.startsWith(`User:${user}/`) && page.endsWith(".js");
    if (!is_own_script)
        return;
    const source_name = new URL(SOURCE_WIKI).hostname;
    mw.loader.using(["mediawiki.util", "mediawiki.api", "oojs-ui-core"], function () {
        const link = mw.util.addPortletLink("p-cactions", "#", `Sync from ${source_name}`, "sync-link");
        ...
        async function fetch_page() {
            ...
            const api_url = `${SOURCE_WIKI}/w/api.php?action=query&titles=${encodeURIComponent(page)}&prop=revisions&rvprop=content&format=json&origin=*&rvslots=*`;
