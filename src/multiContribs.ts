...
(async () => {
  ...
  const parseUsers = (input: string): string[] => {
    const users = input
      .trim()
      .split("\n")
      .map((u) => {
  ...
  const renderContribItem = (c: Contribution): string => {
    ...
    <span class="mw-changeslist-links">
      ...
      <span><a href="/w/index.php?title=${
        c.title
      }&action=history">hist</a></span>
    </span>
    [<a href="/wiki/Special:Contributions/${c.user}" style="font-weight:bold">${
      c.user
    }</a>]
    <bdi><a href="/w/index.php?title=${c.title}&oldid=${
      c.revid
    }" class="mw-changeslist-date">${time}</a></bdi>
    <span class="mw-changeslist-separator"></span>${newFlag}
    <span class="mw-plusminus-pos mw-diff-bytes" style="color:${color};font-weight:${weight}">${sign}${
      c.sizediff || 0
    }</span>
  ...
  class MultiContribs {
    constructor() {
      if (
        mw.config.get("wgNamespaceNumber") !== CONFIG.RUN_NS ||
    ...
    renderUI(): void {
      ...
      const limitsOpts = CONFIG.LIMITS.map(
        (l) => `<option value="${l}">${l}</option>`
      ).join("");
      const nsOpts = CONFIG.NAMESPACES.map(
...
<div class="vector-body">
  ...
  <div id="mctb-results" class="mctb-option"></div>
    ...
    bindEvents(): void {
      getElement("load-contribs")?.addEventListener("click", () =>
    ...
    loadFromURL(): void {
      ...
      if (params.has("tag")) getElement("tag-input").value = params.get("tag")!;
      if (params.has("new"))
    ...
    async loadContributions(): Promise<void> {
      ...
      try {
        ...
        for (const user of users) {
          ...
          if (ns) params.ucnamespace = ns;
          if (tag) params.uctag = tag;
          if (newOnly) params.ucshow = "new";

          const result: any = await new mw.Api().get(params);
          if (result.query?.usercontribs) {
            result.query.usercontribs.forEach((c: Contribution) => {
              c.user = user;
              contribs.push(c);
            });
          }
        ...
        if (contribs.length === 0) {
          ...
          results.innerHTML = `<p>Found ${
            contribs.length
          } contributions</p><ul class="mw-contributions-list">${contribs
            .map(renderContribItem)
            .join("")}</ul>`;
          mw.hook("wikipage.content").fire($(results));
        }
      } catch (error) {
        results.innerHTML = `<p>Error loading contributions: ${
