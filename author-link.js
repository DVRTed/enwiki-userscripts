...
$(() => {
    ...
    class AuthorLink {
        ...
        process_citations() {
            ...
            this.completion_stats.total_authors = this.citation_data.reduce((sum, item) => sum + item.authors.length, 0);
        ...
        extract_authors(citation) {
            ...
            return Object.entries(names)
                ...
                if (num_str === "1") {
                    ...
                    const regex = new RegExp(`\\|\\s*(author-?link${num_str}|author${num_str}-?link)\\s*=\\s*`, "i");
                    if (regex.test(citation)) {
                        return null;
                    }
                }
                const name = (parts.author || `${parts.first || ""} ${parts.last || ""}`).trim();
                return name.length > 2 ? { name, num } : null;
            })
                .filter(Boolean);
        }
        update_title_count() {
            const { completed_authors, total_authors, completed_citations, skipped_citations, total_citations, } = this.completion_stats;
            const status_text = `Authors: ${completed_authors}/${total_authors} | Citations: ${completed_citations + skipped_citations}/${total_citations} (✓ ${completed_citations} — ⚠ ${skipped_citations})`;
            $(".panel-header .completion-counter").remove();
            $(".panel-header").append(`<span class="completion-counter">${status_text}</span>`);
            if (completed_citations + skipped_citations >= total_citations) {
                $(".panel-content").html("<div class='empty-list'>You're all done! <div><button class='generic-button close'>Close</button></div>");
            }
        }
        make_moveable(panel) {
      ...
      <div class="author-link-panel">
        ...
        <div class="panel-content">
         <div class="status">Found ${this.completion_stats.total_authors} author(s) across ${this.citation_data.length} citation(s) missing author-link</div>
          <div class="citations"></div>
            ...
            panel.on("click", ".manual-select-button", (e) => this.handle_manual_click(e));
        ...
        handle_select_click(e) {
            ...
            const author_item = citation_item.authors.find((a) => a.num == author_div.data("num"));
        ...
        highlight_wikitext(wikitext) {
            ...
            html = html.replace(/{{(.*?)\|/, '{{<span class="token-template">$1</span>|');
            html = html.replace(/\|\s*([^=]+)\s*=\s*([^|}]+)/g, (_, param, value) => {
                let token_param = `<span class="token-param">${param}</span>`;
                const reg_match = param.match(/(?:author|authorlink|author-link|first|last)(\d*)/);
                if (reg_match) {
        ...
        <div class="citation" data-index="${i}">
          ...
          <div class="citation-preview">${this.highlight_wikitext(data.citation)}</div>
        </div>
      `).appendTo(container);
                data.authors.forEach((author) => this.process_author(author, citation_div));
            });
        }
        async process_author(author, container) {
            const author_label = author.num === "" ? "author 1" : `author ${author.num}`;
            const author_div = $(`
      <div class="author" data-num="${author.num}">
        <div class="author-name">${mw.html.escape(author.name)}  <span class="author-number">(${author_label})</span></div>
    ...
    `;
                author_div.append(candidates_html || '<div class="no-results">No matches found</div>', manual_input);
            }
            catch (error) {
        ...
        async search_author(name) {
            ...
            const results = (response.query?.search || []).filter((result) => result.title !== current_page);
            return results;
        }
        apply_link(title, citation_item, author) {
            ...
            const updated_citation = citation_item.citation.replace(/(\s*}})$/, ` |${param_name}=${title}$1`);
            if (current_text.includes(citation_item.citation)) {
                const new_text = current_text.replace(citation_item.citation, updated_citation);
        ...
        update_edit_summary() {
            ...
            const changes = `Modified ${this.completion_stats.modified_citations} ${this.completion_stats.modified_citations === 1
                ? "citation"
                : "citations"} ${link}`;
            let new_summary;
            if (!current_summary) {
                ...
                new_summary = current_summary.replace(/Modified \d+ citations?/, `Modified ${this.completion_stats.modified_citations} ${this.completion_stats.modified_citations === 1
                    ? "citation"
                    : "citations"}`);
            }
            else {
        ...
        add_portlet_link() {
            $(mw.util.addPortletLink("p-cactions", "#", "Author links", "ca-author-links")).on("click", (e) => {
                e.preventDefault();
                this.start();
            });
        }
