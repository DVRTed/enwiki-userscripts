// {{Wikipedia:USync |repo=https://github.com/DVRTed/enwiki-userscripts |ref=refs/heads/prod |path=author-link-vue.js}}
"use strict";
/*
author-link.js
...
mw.loader.using(["vue", "@wikimedia/codex"]).then((require) => {
    ...
    if (!TEXTBOX.length)
        return; // if we're not in editing mode, quit
...
<cdx-dialog v-model:open="show_dialog" title="Author Links" :close-button-label="'Close'" class="al-dialog">
      ...
      `,
        computed: {
            ...
            current_stat_text() {
                ...
                return (`Progress: ${processed_cites}/${total} citations ` +
        ...
        methods: {
            init() {
                ...
                this.stats.authors.total = this.citation_data.reduce((sum, item) => sum + item.authors.length, 0);
            ...
            async search_author(author) {
                try {
                    ...
                    author.candidates = (response.query?.search || []).filter((result) => result.title !== current_page);
            ...
            handle_param_click(event, citation) {
                ...
                if (!target.dataset.authorIndex)
                    return;
                const author_index = target.dataset.authorIndex;
                const author = citation.authors.find((a) => (a.index || "1") === author_index);
                if (!author)
                    return;
            ...
            apply_link(title, citation, author) {
                ...
                if (replace_regex.test(citation.cite_text)) {
                    updated_citation = citation.cite_text.replace(replace_regex, `| ${param_name}=${title}`);
                }
                else {
                    updated_citation = citation.cite_text.replace(/(\s*}})$/, ` |${param_name}=${title}$1`);
                }
                // ensure the textarea isn't modified externally after opening the dialog
                if (current_text.includes(citation.cite_text)) {
                    const new_text = current_text.replace(citation.cite_text, updated_citation);
            ...
            update_edit_summary() {
                ...
                const changes = `Modified ${this.stats.citations.modified} ${this.stats.citations.modified === 1 ? "citation" : "citations"} ${link}`;
                let new_summary;
                if (!current_summary) {
                    new_summary = changes;
                }
                else if (current_summary.includes(link)) {
                    new_summary = current_summary.replace(/Modified \d+ citations?/, `Modified ${this.stats.citations.modified} ${this.stats.citations.modified === 1 ? "citation" : "citations"}`);
                }
                else {
            ...
            highlight_wikitext(citation) {
                ...
                html = html.replace(/\|\s*([^=]+)\s*=\s*([^|}]+)/g, (_, param, value) => {
                    ...
                    if (!reg_match) {
                        return (`| <span style="color:#5a5a5a;font-weight:bold">${param}</span>` +
                    ...
                    return (`| <span style="${style}" data-author-index="${author_index}">${param}</span>` +
                        `<span style="color:#7e7e7e">=</span>` +
                        `<span>${value.trim()}</span>`);
                });
                return html;
  ...
  `);
    ...
    const portlet_link = mw.util.addPortletLink("p-cactions", "#", "Author links", "ca-author-links");
